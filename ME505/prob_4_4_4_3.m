...
for m = 0 : n_terms-1
    ...
    for n = 0 : n_terms-1
        ...
        if m == 0
            ...
            fun = @(x,y) x.^2.0.*(1-2./3.*x).*y.*(2-y).*sin((n+1).*pi/2.*y);
            
            C_mn(m+1,n+1) = integral2(fun,0,L,0,M);
            
        else
            fun = @(x,y) x.^2.0.*(1-2./3.*x).*y.*(2-y).*cos(m.*pi.*x).*sin((n+1).*pi/2.*y);
            
            top = integral2(fun,0,L,0,M);
            
            fun_x = @(x)  cos(m.*pi.*x).^2.0;
